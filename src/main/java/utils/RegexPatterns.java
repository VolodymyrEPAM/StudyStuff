...
public class RegexPatterns {
    ...
    public static String getStreetName(String address) {
        String street = Arrays.asList(address.split("•")).get(0);
        return getValueByRegex(street.trim(), streetNamePattern).trim();
    }

    public static String getStreetNumber(String address) {
        String street = Arrays.asList(address.split("•")).get(0);
        return getValueByRegex(street.trim(), streetNumberPattern).trim();
    }

    public static String getSuiteNumber(String address) {
        String street = Arrays.asList(address.split("•")).get(1);
        return getValueByRegex(street.trim(), suiteNumberPattern).trim();
    }

    public static String getStateName(String address) {
        String street = Arrays.asList(address.split(",")).get(1);
        return getValueByRegex(street.trim(), statePattern).trim();
    }

    public static String getPostalCode(String address) {
        String street = Arrays.asList(address.split("•")).get(1);
        return getValueByRegex(street.trim(), postalCodePattern).trim();
    }

    public static String getCountry(String address) {
        String street = Arrays.asList(address.split("•")).get(2);
        return getValueByRegex(street.trim(), countryPattern).trim();
    }
